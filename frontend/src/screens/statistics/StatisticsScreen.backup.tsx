 ...
 *
 * 그룹별 조회 시, 해당 그룹에서 이번 달 본인의 총 지출만 집계합니다.
 * - 로직:
...
export const StatisticsScreen: React.FC<Props> = ({ navigation }) => {
  ...
  /**
   * 그룹/연도/월 변경 시 통계 조회
   * - selectedGroupId가 null이면 개인 전체 통계 조회
   * - selectedGroupId가 있으면 그룹별 통계 조회
   */
  useEffect(() => {
    fetchStatistics();
  ...
  const fetchGroups = async () => {
    try {
      ...
      // 기본적으로 그룹을 선택하지 않음 (개인 전체 통계 표시)
      // selectedGroupId는 null 상태 유지
    } catch (err: any) {
  ...
  const fetchStatistics = async () => {
    try {
      ...
      const isPersonalStats = selectedGroupId === null;
      console.log(
        '[통계] 통계 조회 시작 -',
        isPersonalStats ? '개인 전체 통계' : `그룹: ${selectedGroupId}`,
        '기간:', selectedYear, '년', selectedMonth, '월'
      );
      ...
      const [data, prevData] = await Promise.allSettled([
        isPersonalStats
          ? statisticsApi.getUserTotalStatistics(selectedYear, selectedMonth)
          : statisticsApi.getMonthlyStatistics(selectedGroupId, selectedYear, selectedMonth),
        isPersonalStats
          ? statisticsApi.getUserTotalStatistics(prevYear, prevMonth)
          : statisticsApi.getMonthlyStatistics(selectedGroupId, prevYear, prevMonth),
      ]);
